...
output_reader = function(folder, outroot, out_type){
...
MCMCplot = function(folder, outroot, title="", lag=20){
...
pipost_plotter = function(folder, chan, alpha=0.02, allData=FALSE){
  ...
  if( !allData ){
    for(pat in pts){
      pis[[pat]] = output_reader(folder, paste(chan, pat, sep="_"), out_type="POST")[,"probDef"]
    }
  } else {
